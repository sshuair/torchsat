...
class FC_Siam_Conc(nn.Module):
    def __init__(self, num_classes=1, in_channels=3, pretrained=True):
        ...
        self.decoder5 = DecoderBlock(2048*3, 1024+1024//2, 1024)
        self.decoder4 = DecoderBlock(1024*3, 512+512//2, 512)
        self.decoder3 = DecoderBlock(512*3, 256+256//2, 256)
        self.decoder2 = DecoderBlock(256*3, 128+128//2, 64)  # resnet50 layer1 is only 64 channels
        self.decoder1 = DecoderBlock(64*3, 64+64//2, 32)
    ...
    def forward(self, x_pre: torch.tensor, x_post: torch.tensor):
        ...
        center = self.center(post_encoder5)

        decoder5 = self.decoder5(torch.cat([pre_encoder5, post_encoder5, center], dim=1))
        decoder4 = self.decoder4(torch.cat([pre_encoder4, post_encoder4, decoder5], dim=1))
        decoder3 = self.decoder3(torch.cat([pre_encoder3, post_encoder3, decoder4], dim=1))
        decoder2 = self.decoder2(torch.cat([pre_encoder2, post_encoder2, decoder3], dim=1))
        decoder1 = self.decoder1(torch.cat([pre_encoder1, post_encoder1, decoder2], dim=1))
